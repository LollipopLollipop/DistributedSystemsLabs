...
public class BackgroundSpeaker extends Thread{
	...
	{			
		...
		BlockingQueue<TimeStampedMessage> send_queue = mp.get_send_queue();
		while(true)
		{
			...
			try
			{
				...
				System.out.println("Seng message from " + message.get_source() + " to " +
						message.get_dest() + " with sequence number=" + message.get_seq_num() + 
						" duplicate=" + message.get_duplicate() + " timestamp=" + message.get_timestamp());
				out.flush();
			}
			catch(Exception e)
			{
