	...
	public void run()
	{
		...
		try{
			...
			while(true){
				...
				/*if(message.get_kind().equals("REQUEST_ACK")
						||message.get_kind().equals("REQUEST")
						||message.get_kind().equals("RELEASE")){
					request_release_queue.add(message);
					continue;
				}*/
				/*
				...
				if(multicast==false){
					...
					if(((TimeStampedMessage)message).get_sent_to_me()==true){
						//this.clockservice.check_and_update(message,1);
						this.clockservice.check_and_update((TimeStampedMessage)message,0);
					}
					else{
						this.clockservice.check_and_update((TimeStampedMessage)message,0);
						continue;
				...
				if(rule == null){
					...
					if(message.get_kind().equals("REQUEST_ACK")
							||message.get_kind().equals("REQUEST")
							||message.get_kind().equals("RELEASE")){
						mp.get_request_release_queue().add(message);
					}
					else 
						mp.get_receive_queue().add(message);
					...
					while(!mp.get_delayed_receive_queue().isEmpty()){
						...
						Message delayed_msg = mp.get_delayed_receive_queue().poll();
						if(delayed_msg.get_kind().equals("REQUEST_ACK")
								||delayed_msg.get_kind().equals("REQUEST")
								||delayed_msg.get_kind().equals("RELEASE")){
							mp.get_request_release_queue().add(delayed_msg);
						}
						else 
							mp.get_receive_queue().add(delayed_msg);
					}
				}
				else{
					...
					if(message.get_kind().equals("REQUEST_ACK")
						...
						if(rule.get_action().equals("duplicate"))
							mp.get_request_release_queue().add(message.copy());
					}
					else{
						mp.get_receive_queue().add(message);
						if(rule.get_action().equals("duplicate"))
