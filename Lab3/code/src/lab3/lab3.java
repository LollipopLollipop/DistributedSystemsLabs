...
public class lab3
{
	public static String check_action(String input)
	{
		...
		if((!(action.equals("receive")) && !(action.equals("send")) && 
				...
				!(action.equals("multicast")) && !(action.equals("status")) &&
				!(action.equals("request")) && !(action.equals("release")) &&
				!(action.equals("record"))))
		{
			return null;
		}
	...
	public static void main(String[] args) throws IOException
	{
		...
		//oChecker.start();
		MutualExclusionCheck mChecker = new MutualExclusionCheck(mp);
		mChecker.start();
		CheckThread checkthread = new CheckThread(mp);
		checkthread.start();
		...
		while(true)
		{
			...
			if((action = check_action(input)) != null)
			{
				...
				{
					TimeStampedMessage message = (TimeStampedMessage)mp.receive();
					if(message == null){
